...
~/.gbrain/
  ...
  artifacts/         # Artifact 原件存储（按 SHA256 去重命名）
  kb/                # KB 子系统存储
    files/           # KB 文档文件（按库 ID 分目录）
  cache/             # 缓存目录
  logs/              # 日志文件
    gbrain.log
```
...
| 子命令 | 说明 |
|--------|------|
| `gbrain config show` | 显示所有常用配置值 |
| `gbrain config get <key>` | 获取单个配置值 |
...
| `chunker_model` | string | LLM 分块模型 | `gpt-4o-mini` |
...
- **KB 内部工具**（`kb_list_libraries`, `kb_upload_document`, `kb_search` 等）需通过 `expose_internal_tools=true` 开启，默认不可见。面向用户的日常操作请使用 `artifact_*` facade 工具。

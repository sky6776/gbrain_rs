...
**个人知识大脑引擎** — 基于 [gbrain](https://github.com/garrytan/gbrain) 的 Rust 移植，新增单入口多投影融合架构（Artifact 原件 → KB/影子页面/候选变更/附件多投影 + 溯源审计 + 回滚）、KB 子系统（异步文档处理管线 + 新库默认启用的 RAPTOR 递归摘要树）、中文 NLP 全链路支持（jieba 分词 + 拼音 + FTS5 查询重构）、软删除生命周期、脑页检索的时间衰减加权等特性。基于 SQLite + FTS5 的嵌入式架构；向量检索在可用时使用 sqlite-vec，否则回退到内置 BLOB 存储检索，无需外部数据库。
...
gbrain 使用嵌入式 SQLite，不需要外部数据库。`gbrain init`、`gbrain config show/get/set`（Config 字段）以及支持预览语义的 dry-run 路径会尽量跳过完整外部模型校验；`serve`、`query`、`list/get/health`、非预览写入/上传、KB/OCR 等真实连库命令会在运行前严格校验存储、LLM、OCR、KB、日志等环境变量。关键词检索本身不调用外部模型，嵌入向量、查询扩展、重排序、RAPTOR、OCR 等外部能力按各自配置调用。

---

## 特性

- **检索能力** — Artifact 统一 facade 查询当前以 FTS5 关键词检索为主；底层 hybrid 库 API 在提供查询向量时可用 RRF 融合关键词与向量结果，并让预计算的 `expanded_queries` 作为额外向量结果列表参与 RRF；标题级 trigram 模糊匹配用于链接解析，全文模糊搜索仅作为库 API 暴露
- **知识图谱** — Wiki 链接提取、类型化链接、图遍历、反向链接对称性验证
- **KB 子系统** — 异步文档处理管线，新建库默认启用 RAPTOR（文档节点数满足要求时执行，LLM 只读取 `GBRAIN_KB_RAPTOR_*`）；文档拆分采用 adaptive 策略，有嵌入器时大块可做语义细分；支持 Markdown/PDF/DOCX/XLS/XLSX/CSV/HTML/纯文本解析、PDF 页级 OCR 自动检测与回写，以及 JPG/PNG 图片上传后的自动 OCR 入库；代码页面索引走独立代码流程
...
- **嵌入式存储** — 基于 SQLite/FTS5，无需外部数据库；真实连库命令运行前需按“环境变量”章节提供完整配置
- **分层丰富** — 库 API 支持实体检测、存根页创建、反向链接，以及层级/标签/链接建议
- **版本历史** — 页面覆盖前会记录版本快照，库 API 支持版本回滚；建议变更回滚已通过 `gbrain review rollback` 暴露
...
- **安全防护** — 路径遍历防护、slug 验证、MCP 远程文件路径约束、参数化查询防 SQL 注入
...
- **软删除生命周期** — Artifact 删除/恢复已暴露；Artifact 原件的永久清理目前未实现，页面与 KB 文档有独立清理路径
...
| `gbrain kb ocr-retry <doc_id>` | 重试失败或空结果的 OCR 页 |
| `gbrain kb mine-synonyms [--library-id] [--full]` | 离线挖掘 token 语义同义词 |
...
|------|------|------|------|
| `path` | path | 是 | 本地文件路径 |
| `--intent` | string | 否 | 上传意图: auto(默认；提升策略默认取 `upload_default_promotion_policy`，当前为 `auto-apply`), evidence(别名 document), memory, attachment, promote |
...
| `--promotion` | string | 否 | 提升策略: none, shadow, candidate, auto-low-risk(alias auto), auto-apply |
...
| `ocr-run --pages <RANGES>` | string | 否 | 运行 OCR 的页码范围，如 `1,3,5-10` |
| `mine-synonyms` | command | 否 | 基于 token embedding 离线挖掘语义同义词 |
| `mine-synonyms --library-id <ID>` | integer | 否 | 仅挖掘指定 KB 库 |
| `mine-synonyms --full` | flag | 否 | 全量重挖，忽略已有 token embedding |
...
| 子命令 | 说明 |
|--------|------|
| `gbrain config show` | 显示常用配置值的快速概览 |
| `gbrain config get <key>` | 获取单个配置值（可查询以下 Config 字段） |
| `gbrain config set <key> <value>` | 设置配置值（另支持两个仅可设置的 OCR 阈值 key） |

> **注意:** `config show` 只做快速概览；`config get <key>` 支持查询下表中的 Config 字段。`writer.lint_on_put_page` 是 SQLite engine 专用 key，会在连接数据库后读取。`config set` 还接受 `ocr_text_density_threshold` 与 `ocr_timeout_seconds_per_page`，它们会写入 `config.json`，但当前不能通过 `config get` 读取。
...
| `chunker_model` | string | 普通脑页 LLM 引导分块模型；KB 文档处理不使用该分块器 | `gpt-4o-mini` |
| `page_chunker_mode` | string | 普通脑页分块模式: auto/recursive/llm；代码页仍追加 tree-sitter 符号分块 | `auto` |
...
| `kb_raptor_model` | string | KB RAPTOR LLM 模型；启动时由 `GBRAIN_KB_RAPTOR_MODEL` 显式提供 | 空字符串 |
...
| `upload_default_promotion_policy` | string | 上传默认提升策略: none/shadow/candidate/auto-low-risk/auto-apply | `auto-apply` |
...
| `ocr_enabled` | boolean | 启用 PDF/图片 OCR | `true` |
...
| `ocr_submit_mode` | string | OCR 提交策略: pdf_first/pdf_range | `pdf_range` |
...
gbrain config set chunk_size 800
gbrain config set log_level debug
gbrain config set upload_default_promotion_policy auto-apply
```
...
- slug 格式校验（防路径遍历）
- 文件读取路径必须位于 MCP 服务进程的当前工作目录内，且不得包含符号链接
- 上传文件扩展名、MIME 类型与大小校验
- 参数化查询（防 SQL 注入）
- 文件名安全检查

CLI 使用 `remote=false`，不应用 MCP 专有的当前工作目录与符号链接限制。
...
涉及文件的 MCP 请求只能读取服务进程当前工作目录内已经存在的非符号链接文件；下方 `./imports/...` 路径相对于启动 `gbrain serve` 的目录。
...
// 按查询词读取聚焦片段，避免拉取整篇原文
{ "tool": "artifact_get", "params": { "id_or_uid": "art_abc123", "content_mode": "focused", "content_query": "Rust 异步", "max_chars": 1600 } }
...
| 策略值 | 别名 | 说明 |
|--------|------|------|
| `none` | — | 不触发 promotion 抽取；上传路由仍可按 intent 创建 KB/影子页 |
| `shadow` | — | 仅创建影子页，不生成候选变更 |
| `candidate` | — | 生成候选变更，需人工审核 |
| `auto-low-risk` | — | 自动接受低风险候选（实体提及、链接建议等），高风险仍需审核 |
| `auto-apply` | `auto_apply` | 自动接受并应用所有候选；候选记录保留用于审计和回滚（默认行为） |

说明：`upload`/`artifact_upload` 未显式传 `--promotion`/`promotion` 时使用 `upload_default_promotion_policy`（默认 `auto-apply`）；手动 `put`/`artifact_put` 的 `memory` intent 仍按低风险自动应用策略执行。
...
| `intent` | string | 否 | 上传意图: auto / evidence(别名 document) / memory / attachment / promote（默认 auto；提升策略默认取 `upload_default_promotion_policy`，当前为 `auto-apply`） |
...
| `promotion` | string | 否 | 提升策略: none / shadow / candidate / auto-low-risk / auto-apply |
| `dry_run` | boolean | 否 | 仅返回路由计划，不实际写入 |
...
| `include_content` | boolean | 否 | 包含原始内容；默认建议优先使用 `content_mode=focused` 或 `artifact_query` 片段 |
| `content_mode` | string | 否 | 正文读取模式：`focused`（按 query/passage 取片段）或 `full`（全文，需 `include_content=true`） |
| `content_query` | string | 否 | `focused` 模式的查询词 |
| `max_chars` | integer | 否 | `focused` 内容最大字符数，默认 1600，内部限制在 200..=10000 |
| `passage_id` | integer | 否 | 直接读取 `artifact_query` 返回的 KB passage ID |
...
| `change_id` | integer | 是 | 变更 ID |
| `reason` | string | 否 | 拒绝原因 |
| `reviewer` | string | 否 | 审核者标识；默认值为 `mcp` |
...
> **API 兼容性说明**: 除 PDF/图片 OCR 使用智谱 GLM-OCR `layout_parsing` endpoint 外，本项目的通用模型与音频调用仅支持 OpenAI 兼容格式的 API（`/embeddings`、`/chat/completions`、`/audio/transcriptions`），不支持 Anthropic/Claude API。通过设置相应 `*_BASE_URL` 可接入兼容服务；OCR endpoint 由单独的安全开关控制。

### LLM 配置分组

LLM 配置按调用类型和功能拆分。除 `init`、Config 字段读写和支持预览语义的 dry-run 快速路径外，真实连库命令会执行完整运行时校验：必填项缺失、空值、URL 不合法、枚举/布尔/数字格式不合法都会直接报错并终止命令。

| 配置组 | 环境变量 | 适用功能 |
|--------|----------|----------|
| **Embeddings** | `GBRAIN_EMBEDDING_API_KEY` / `GBRAIN_EMBEDDING_BASE_URL` / `GBRAIN_EMBEDDING_MODEL` / `GBRAIN_EMBEDDING_DIMENSIONS` | 文档分块嵌入（向量化）、语义分块（段落相似度切分）、查询向量 |
| **查询扩展 / 重排序** | `GBRAIN_EXPANSION_API_KEY` / `GBRAIN_EXPANSION_BASE_URL` / `GBRAIN_EXPANSION_MODEL` | 查询扩展、通过 chat/completions 进行搜索重排序 |
| **KB RAPTOR** | `GBRAIN_KB_RAPTOR_API_KEY` / `GBRAIN_KB_RAPTOR_BASE_URL` / `GBRAIN_KB_RAPTOR_MODEL` | RAPTOR 树摘要，以及复用 RAPTOR LLM 配置的关键词/问题增强 |
| **普通脑页 LLM 分块** | `GBRAIN_CHUNKER_API_KEY` / `GBRAIN_CHUNKER_BASE_URL` / `GBRAIN_CHUNKER_MODEL` / `GBRAIN_PAGE_CHUNKER_MODE` | `pages/chunks` 的普通脑页 LLM 引导分块；KB 文档处理仍使用 KB 自适应 splitter |
| **音频转录** | `GBRAIN_TRANSCRIPTION_PROVIDER` + 对应 provider 的 API key/base URL | 库模块中的音频转录能力 |
| **PDF/图片 OCR（GLM-OCR）** | `GBRAIN_OCR_API_KEY` / `GBRAIN_OCR_BASE_URL` / `GBRAIN_OCR_MODEL` | PDF 页级版面识别、JPG/PNG 单图识别、文本回写与重嵌入 |

### 环境变量不再共享或回退

外部模型配置必须按用途显式提供，不再使用共享 API key、共享 OpenAI 兼容基础 URL 或旧兼容别名：

- Embeddings 只读取 `GBRAIN_EMBEDDING_*`。
- 查询扩展、搜索重排序和对话改写只读取 `GBRAIN_EXPANSION_*`。
- 普通脑页 LLM 分块只读取 `GBRAIN_CHUNKER_*`，分块模式由 `GBRAIN_PAGE_CHUNKER_MODE` 控制。
- RAPTOR 只读取 `GBRAIN_KB_RAPTOR_*`；不再从库级 `raptor_llm_*`、`GBRAIN_EXPANSION_*` 或 `GBRAIN_CHUNKER_*` 回退。
- OCR 只读取 `GBRAIN_OCR_*`；不再兼容 `ZHIPU_API_KEY`、`GLMOCR_MODEL`、`GLMOCR_ENABLE_LAYOUT`、`GLM_OCR_TIMEOUT` 或 `GLMOCR_TIMEOUT`。
- 旧共享前缀 `GBRAIN_OPENAI_*` 不再作为任何模块的回退配置。

### 基础配置

| 变量 | 说明 | 启动要求 |
|------|------|----------|
| `GBRAIN_DIR` | 数据存储根目录 | 必填，非空 |
| `GBRAIN_DB_PATH` | SQLite 数据库文件路径 | 未通过 CLI `--db` 或 config.json `database_path` 设置数据库路径时必填，非空 |

### 嵌入向量

| 变量 | 说明 | 启动要求 |
|------|------|----------|
| `GBRAIN_EMBEDDING_API_KEY` | 嵌入向量 API 密钥 | 必填，非空 |
| `GBRAIN_EMBEDDING_BASE_URL` | OpenAI 兼容 embeddings 基础 URL | 必填，合法 http/https URL |
| `GBRAIN_EMBEDDING_MODEL` | 嵌入模型名称 | 必填，非空 |
| `GBRAIN_EMBEDDING_DIMENSIONS` | 嵌入向量维度 | 必填，非零整数 |
| `GBRAIN_EMBEDDING_CONCURRENCY` | embedding 子批次并发请求数，上限 8 | 可选；启动不校验，无法解析时按 `1`，最终限制在 `1..=8` |

### 查询扩展 / 重排序（Chat Completions）

| 变量 | 说明 | 启动要求 |
|------|------|----------|
| `GBRAIN_EXPANSION_API_KEY` | 查询扩展、搜索重排序、对话改写的 LLM API 密钥 | 必填，非空 |
| `GBRAIN_EXPANSION_BASE_URL` | 查询扩展、搜索重排序、对话改写的 OpenAI 兼容基础 URL | 必填，合法 http/https URL |
| `GBRAIN_EXPANSION_MODEL` | 查询扩展、搜索重排序、对话改写的 LLM 模型 | 必填，非空 |

### 普通脑页 LLM 分块

普通脑页（`pages/chunks`）默认使用 `GBRAIN_PAGE_CHUNKER_MODE=auto`：非代码页面优先尝试 `src/chunker/llm.rs` 的 LLM 引导分块，不适合或失败时回退递归分块；代码页仍保留 deterministic 分块并追加 tree-sitter 符号 chunk。KB 文档处理不使用这套 LLM 分块器，仍走 `src/kb/splitter`。

| 变量 | 说明 | 启动要求 |
|------|------|----------|
| `GBRAIN_CHUNKER_API_KEY` | LLM 分块 API 密钥 | 必填，非空 |
| `GBRAIN_CHUNKER_BASE_URL` | LLM 分块 OpenAI 兼容基础 URL | 必填，合法 http/https URL |
| `GBRAIN_CHUNKER_MODEL` | LLM 分块模型 | 必填，非空 |
| `GBRAIN_PAGE_CHUNKER_MODE` | 普通脑页分块模式：`auto` / `recursive` / `llm`；未配置时默认 `auto` | 可选；若配置必须是合法枚举 |
...
| 变量 | 说明 | 启动要求 |
|------|------|----------|
| `GBRAIN_TRANSCRIPTION_PROVIDER` | 转录服务提供商（`groq` / `openai`） | 必填；只能是 `groq` 或 `openai` |
| `GBRAIN_TRANSCRIPTION_GROQ_API_KEY` | Groq 转录 API 密钥 | provider=`groq` 时必填；若配置则不能为空 |
| `GBRAIN_TRANSCRIPTION_GROQ_BASE_URL` | Groq 转录基础 URL | provider=`groq` 时必填合法 URL；若配置则必须合法 |
| `GBRAIN_TRANSCRIPTION_OPENAI_API_KEY` | OpenAI 转录 API 密钥 | provider=`openai` 时必填；若配置则不能为空 |
| `GBRAIN_TRANSCRIPTION_OPENAI_BASE_URL` | OpenAI 转录基础 URL | provider=`openai` 时必填合法 URL；若配置则必须合法 |
...
JPG/PNG 上传会直接作为单页 OCR 文档入队，识别结果回写为 KB 文本节点并触发重嵌入。单图按 GLM-OCR 限制最大 10 MiB；图片没有 PDF 的原生文本层，若 OCR 被关闭或缺少 API key，则不会有本地文本 fallback。

外部 OCR 始终允许（库级隐私开关已移除）。只要全局开关 `GBRAIN_OCR_ENABLED=true` 且配置了 API key，PDF 页面和 JPG/PNG 图片就会发送至外部 OCR 服务。

| 变量 | 说明 | 启动要求 |
|------|------|----------|
| `GBRAIN_OCR_ENABLED` | 启用 PDF/图片 OCR 总开关 | 必填布尔值 |
| `GBRAIN_OCR_API_KEY` | GLM-OCR API 密钥 | `GBRAIN_OCR_ENABLED=true` 时必填 |
| `GBRAIN_OCR_BASE_URL` | GLM-OCR endpoint；自定义地址还必须显式开启下方安全开关 | `GBRAIN_OCR_ENABLED=true` 时必填合法 URL |
| `GBRAIN_OCR_ALLOW_CUSTOM_BASE_URL` | 是否允许使用 `GBRAIN_OCR_BASE_URL` 中的自定义 endpoint；为 `false` 时实际请求使用官方默认 endpoint | 必填布尔值 |
| `GBRAIN_OCR_MODEL` | OCR 模型名称 | `GBRAIN_OCR_ENABLED=true` 时必填 |
| `GBRAIN_OCR_PROFILE` | 后处理 profile：`auto` / `general` / `table` / `formula` / `handwriting`；`auto` 会按 OCR 版面块自动增强表格和公式，不发送给 API | 必填枚举值 |
| `GBRAIN_OCR_ENABLE_LAYOUT` | 请求版面识别；`GBRAIN_OCR_PROFILE=auto` 建议开启 | 必填布尔值 |
| `GBRAIN_OCR_MODE` | OCR 选择模式：`auto` / `all_pages` | 必填枚举值 |
| `GBRAIN_OCR_SUBMIT_MODE` | PDF 提交模式：`pdf_first` / `pdf_range`；GLM-OCR payload 会强制使用安全的 `pdf_range` 流程 | 必填枚举值 |
| `GBRAIN_OCR_SYNC_INLINE` | 是否内联执行 OCR；`false` 使用后台异步 job | 必填布尔值 |
| `GBRAIN_OCR_TEXT_DENSITY_THRESHOLD` | 低文本密度页字符数阈值 | 必填非负整数 |
| `GBRAIN_OCR_MIN_LOW_DENSITY_RATIO` | 低密度比例兼容配置；当前只保留统计信息，不否决单页 OCR 选择 | 必填 0..=1 有限数字 |
| `GBRAIN_OCR_IMAGE_AREA_THRESHOLD` | 图片面积覆盖率触发阈值 | 必填 0..=1 有限数字 |
| `GBRAIN_OCR_IMAGE_COUNT_THRESHOLD` | 嵌入图片数量触发阈值 | 必填非零整数 |
| `GBRAIN_OCR_TIMEOUT_SECONDS_PER_PAGE` | 单页请求超时秒数 | 必填非零整数 |
| `GBRAIN_OCR_MAX_PAGES_PER_REQUEST` | 单次 OCR 请求最大页数；GLM-OCR 路径实际会限制在 1..=5 | 必填非零整数 |
| `GBRAIN_OCR_MAX_PDF_BYTES_PER_REQUEST` | 单次 OCR 请求最大 PDF 字节数；`52,428,800` = 50 MiB | 必填非零整数 |
| `GBRAIN_OCR_MAX_PAGES_PER_DOCUMENT` | 单文档最多尝试 OCR 的页数 | 必填非零整数 |
| `GBRAIN_OCR_MAX_CONCURRENCY` | 进程内 OCR 并发工作上限 | 必填非零整数 |
| `GBRAIN_OCR_TEMP_DIR_MAX_BYTES` | OCR 拆分页临时文件的进程内总字节预算；`536,870,912` = 512 MiB | 必填非零整数 |
| `GBRAIN_OCR_RETURN_CROP_IMAGES` | 请求返回裁剪图片 | 必填布尔值 |
| `GBRAIN_OCR_NEED_LAYOUT_VISUALIZATION` | 请求返回版面可视化结果 | 必填布尔值 |

**安全说明：**

- OCR 只读取 `GBRAIN_OCR_*` 环境变量；旧别名不会生效。
...
| 变量 | 说明 | 启动要求 |
|------|------|----------|
| `GBRAIN_KB_ENABLED` | 启用 KB 子系统 | 必填布尔值 |
| `GBRAIN_KB_RAPTOR_API_KEY` | KB RAPTOR LLM API 密钥 | 必填，非空 |
| `GBRAIN_KB_RAPTOR_BASE_URL` | KB RAPTOR LLM OpenAI 兼容基础 URL | 必填，合法 http/https URL |
| `GBRAIN_KB_RAPTOR_MODEL` | KB RAPTOR LLM 模型 | 必填，非空 |
| `GBRAIN_KB_MAX_FILE_SIZE_MB` | KB 文件大小上限（MB） | 必填非零整数 |
| `GBRAIN_KB_ALLOWED_EXTENSIONS` | KB 允许的文件扩展名（逗号分隔，不能含空项） | 必填非空列表 |
| `GBRAIN_KB_STORAGE_DIR` | KB 文件存储目录 | 必填，非空 |
| `GBRAIN_KB_WORKER_ENABLED` | 启用 KB 后台处理 worker | 必填布尔值 |
| `GBRAIN_KB_WORKER_POLL_INTERVAL` | KB worker 轮询间隔（秒） | 必填非零整数 |
| `GBRAIN_AUTOPILOT_ENABLED` | 启用 autopilot 后台维护线程（`gbrain serve` 时生效） | 必填布尔值 |
| `GBRAIN_AUTOPILOT_INTERVAL` | autopilot 维护间隔（秒） | 必填整数，最小 60 |

**KB 子系统 LLM 用途说明：**

| 功能 | LLM 类型 | API Key / Base URL | 使用模型 |
|------|----------|-------------------|----------|
| 文档分块嵌入（向量化） | Embeddings API | `GBRAIN_EMBEDDING_API_KEY` / `GBRAIN_EMBEDDING_BASE_URL` | `GBRAIN_EMBEDDING_MODEL` |
| 语义分块（段落相似度切分） | Embeddings API | `GBRAIN_EMBEDDING_API_KEY` / `GBRAIN_EMBEDDING_BASE_URL` | `GBRAIN_EMBEDDING_MODEL` |
| PDF 页级/图片 OCR 与文本回写 | GLM-OCR `layout_parsing` | `GBRAIN_OCR_API_KEY` / `GBRAIN_OCR_BASE_URL`（自定义地址需 `GBRAIN_OCR_ALLOW_CUSTOM_BASE_URL=true`） | `GBRAIN_OCR_MODEL` |
| RAPTOR 层级摘要 | Chat Completions | `GBRAIN_KB_RAPTOR_API_KEY` / `GBRAIN_KB_RAPTOR_BASE_URL` | `GBRAIN_KB_RAPTOR_MODEL` |
| 搜索重排序（Rerank） | Chat Completions；未配置/失败时本地重排 | `GBRAIN_EXPANSION_API_KEY` / `GBRAIN_EXPANSION_BASE_URL` | `GBRAIN_EXPANSION_MODEL` |

### Artifact 融合架构

| 变量 | 说明 | 启动要求 |
|------|------|----------|
| `GBRAIN_ARTIFACT_STORAGE_DIR` | Artifact 原件存储目录 | 必填，非空 |
| `GBRAIN_DEFAULT_KB_LIBRARY_ID` | 默认 KB 库 ID；可为空表示不指定 | 可选整数或空字符串 |
| `GBRAIN_UPLOAD_PROMOTION_POLICY` | 上传默认提升策略: none/shadow/candidate/auto-low-risk/auto-apply/auto_all 等别名 | 必填合法策略 |
| `GBRAIN_ARTIFACT_DEFAULT_INTENT` | artifact 默认意图: memory/evidence/promote | 必填枚举值 |
| `GBRAIN_ARTIFACT_AUTO_CREATE_INBOX_LIBRARY` | artifact_put 无 Inbox 库时自动创建 | 必填布尔值 |
| `GBRAIN_ARTIFACT_MANUAL_MEMORY_TO_KB` | memory 意图是否写入 KB（设为 `false` 仅写入 gbrain 页面） | 必填布尔值 |

### 日志

| 变量 | 说明 | 启动要求 |
|------|------|----------|
| `GBRAIN_LOG_LEVEL` | 日志级别（trace/debug/info/warn/error） | 必填枚举值 |
| `GBRAIN_LOG_TO_FILE` | 启用文件日志 | 必填布尔值 |
| `GBRAIN_LOG_FILE_PATH` | 日志文件路径 | `GBRAIN_LOG_TO_FILE=true` 时必填 |
| `GBRAIN_LOG_TO_CONSOLE` | 启用控制台日志 | 必填布尔值 |
| `GBRAIN_RUST_LOG` | gbrain 专用 tracing 过滤规则；优先级高于 `GBRAIN_LOG_LEVEL`，支持模块规则如 `gbrain_core::kb::pipeline=debug` | 可选；若配置则必须非空且是合法 EnvFilter 规则 |

### 行为控制

| 变量 | 说明 | 启动要求 |
|------|------|----------|
| `GBRAIN_AUTO_LINK` | 写入时自动提取链接 | 必填布尔值 |
| `GBRAIN_AUTO_TIMELINE` | 写入时自动提取时间线 | 必填布尔值 |
| `GBRAIN_POST_WRITE_LINT` | 写入后运行 validators 检查并记录日志 | 必填布尔值 |

### 调试

| 变量 | 说明 | 启动要求 |
|------|------|----------|
| `GBRAIN_SEARCH_DEBUG` | 启用搜索调试日志（`true`/`false`/`1`/`0`） | 可选；若配置必须是布尔值 |
| `GBRAIN_PROGRESS_MODE` | 进度显示模式（`auto` / `human` / `json` / `quiet`） | 可选；若配置必须是合法枚举 |
| `GBRAIN_PROGRESS_JSON` | 强制 JSON 进度输出（`true`/`false`/`1`/`0`） | 可选；若配置必须是布尔值 |
| `GBRAIN_ASYNC_WORKER_THREADS` | 全局 Tokio runtime 工作线程数 | 可选；若配置必须是非零整数 |
| `GBRAIN_SOURCE_BOOST` | 搜索来源加权规则，格式为 `前缀:系数`，逗号分隔 | 可选；系数必须是非负有限数 |
| `GBRAIN_SEARCH_EXCLUDE` | 搜索时额外硬排除的 slug 前缀，逗号分隔；默认已排除 `test/`、`archive/`、`attachments/`、`.raw/` | 可选；启动不校验，空项会被忽略 |
...
```
正常知识源 ──delete──→ 软删除状态（仍占存储，不可查询）
                           │
                           └──restore──→ 恢复为正常知识源
```

Artifact 原件目前没有永久清理操作。引擎中的页面或 KB 文档清理路径不会将 `source_artifacts` 原件推进到 `purged` 状态。

- `gbrain delete <id_or_uid>` — 软删除，数据保留
- `gbrain restore <id_or_uid>` — 恢复软删除的知识源
- `gbrain health` — 检查知识源一致性
...
2. **拆分** — adaptive splitter 结构优先；表格/代码使用专门逻辑，普通大块在具备嵌入器时可使用语义拆分器（Savitzky-Golay 平滑 + chunk_overlap 重叠），否则回退递归拆分
3. **嵌入** — 向量嵌入生成与持久化
4. **RAPTOR（新库默认开启）** — `raptor_enabled=true` 且节点数量满足条件时构建递归摘要树（K-Means++ 聚类 + LLM 摘要）；LLM 只使用显式的 `GBRAIN_KB_RAPTOR_*` 配置
5. **持久化** — 事务保护的节点/向量写入

`raptor_enabled` 存放在每条 `kb_libraries` 记录中；新建库及自动创建的 `Inbox` 库默认均为 `true`。完整运行时校验会要求 `GBRAIN_KB_RAPTOR_API_KEY`、`GBRAIN_KB_RAPTOR_BASE_URL` 与 `GBRAIN_KB_RAPTOR_MODEL`；处理管线在文档节点少于 3 个、有向量节点不足、RAPTOR LLM 配置解析失败或摘要调用失败时会跳过/继续，不让摘要树失败阻断基础索引。当前 `gbrain config`、CLI 的 `kb` 子命令和 MCP 工具均未暴露关闭开关。

对于 PDF，解析阶段会先生成页级 OCR 判定：空/低密度文本层、图片与矢量内容、批注 appearance 风险、隐藏文本、字体编码疑似异常以及解析/几何不确定页都会保守进入候选集合。需要 OCR 时，系统默认排入异步 OCR job，识别完成后回写文本并触发重嵌入；仅在 `GBRAIN_OCR_SYNC_INLINE=true` 时在处理管线中内联执行。对于 JPG/PNG，上传时直接建立单页 OCR 文档并入队，OCR 完成后再生成 KB 节点。
...
- **Artifact** — 上传的原始文件，当前可达状态为 active/deleted（`purged` 为预留状态）、来源类型（upload/sync/link/mcp）、上传意图（auto/evidence/memory/attachment/promote；`document` 是 `evidence` 的别名）

...
外部 OCR 还受库级隐私策略约束：只有 `external_ocr_allowed=true` 且 `redaction_enabled=false` 时，PDF 页面和 JPG/PNG 图片才会发送至外部 OCR 服务。否则 PDF 会保留可提取的原始文本层，图片则标记为需要/失败并等待策略或凭证恢复后重试。

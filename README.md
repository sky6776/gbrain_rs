...
- **KB 子系统** — 异步文档处理管线，新建库默认启用 RAPTOR（满足模型配置与库策略时执行）；语义分块仍按库配置启用；支持 Markdown/PDF/DOCX/XLSX/CSV/HTML/纯文本解析、PDF 页级 OCR 自动检测与回写，以及 JPG/PNG 图片上传后的自动 OCR 入库；代码页面索引走独立代码流程
...
PDF/图片 OCR: GBRAIN_OCR_API_KEY → ZHIPU_API_KEY
```

设置 `GBRAIN_OPENAI_API_KEY` 可启用嵌入向量、查询扩展和搜索重排序（使用 OpenAI 兼容默认端点/模型）。RAPTOR 需要库级/KB 级 RAPTOR secret，或配置 `GBRAIN_EXPANSION_API_KEY` / `GBRAIN_CHUNKER_API_KEY`。PDF/图片 OCR 使用独立的 GLM-OCR 凭证，不会回退到 `GBRAIN_OPENAI_API_KEY`。
...
JPG/PNG 上传会直接作为单页 OCR 文档入队，识别结果回写为 KB 文本节点并触发重嵌入。单图按 GLM-OCR 限制最大 10 MiB；图片没有 PDF 的原生文本层，若 OCR 被关闭、缺少 API key 或被库策略禁止，则不会有本地文本 fallback。

外部 OCR 始终允许（库级隐私开关已移除）。只要全局开关 `GBRAIN_OCR_ENABLED=true` 且配置了 API key，PDF 页面和 JPG/PNG 图片就会发送至外部 OCR 服务。

| 变量 | 说明 | 默认值 |
|------|------|--------|
| `GBRAIN_OCR_ENABLED` | 启用 PDF/图片 OCR 总开关 | `true` |
...
| `GBRAIN_KB_ALLOWED_EXTENSIONS` | KB 允许的文件扩展名（逗号分隔） | `pdf,docx,xlsx,csv,html,htm,txt,md,markdown,rst,json,xml,yaml,yml,toml,tsv,png,jpg,jpeg` |
...
| `GBRAIN_UPLOAD_PROMOTION_POLICY` | 上传默认提升策略: none/shadow/candidate/auto-low-risk/auto-apply | `auto-apply` |

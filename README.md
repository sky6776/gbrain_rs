...
- **MCP 服务器** — 完整的模型上下文协议（JSON-RPC 2.0）服务器，默认暴露 Artifact facade 工具；开启 internal tools 后可访问完整工具集
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `--status` | string | 否 | 按状态过滤（pending/applied/rejected） |

### `gbrain artifact review apply`

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `change_id` | string | 是 | 建议变更 ID |
...
### Admin/Legacy CLI 参数（需 admin-tools feature）

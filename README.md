...
gbrain 使用嵌入式 SQLite，不需要外部数据库；但当前启动会严格校验存储、LLM、OCR、KB、日志等环境变量。关键词检索本身不调用外部模型，嵌入向量、查询扩展、重排序、RAPTOR、OCR 等外部能力按各自配置调用。
...
- **嵌入式存储** — 基于 SQLite/FTS5，无需外部数据库；启动前需按“环境变量”章节提供完整配置
...
LLM 配置按调用类型和功能拆分。当前启动会严格校验这些环境变量：必填项缺失、空值、URL 不合法、枚举/布尔/数字格式不合法都会直接报错并终止启动。
...
| `GBRAIN_DB_PATH` | SQLite 数据库文件路径 | 必填，非空 |
...
`raptor_enabled` 存放在每条 `kb_libraries` 记录中；新建库及自动创建的 `Inbox` 库默认均为 `true`。当前启动阶段会强制校验 `GBRAIN_KB_RAPTOR_API_KEY`、`GBRAIN_KB_RAPTOR_BASE_URL` 与 `GBRAIN_KB_RAPTOR_MODEL`；处理管线只会在文档节点少于 3 个等运行条件不足时跳过摘要树构建。当前 `gbrain config`、CLI 的 `kb` 子命令和 MCP 工具均未暴露关闭开关。

...
- **KB 子系统** — 异步五阶段文档处理管线（解析→拆分→嵌入→RAPTOR→持久化），RAPTOR 递归摘要树，文档上传与处理，多格式解析器（Markdown/PDF/DOCX/XLSX/CSV/HTML/纯文本/代码），PDF 页级 OCR 自动检测与回写，语义分块（Savitzky-Golay 平滑 + chunk_overlap 重叠）
...
~/.gbrain/
  ...
  kb/                # KB 子系统元数据存储
  cache/             # 缓存目录
  kb_files/          # KB 文件存储（仅通过 GBRAIN_KB_STORAGE_DIR 自定义时生效）
  logs/              # 日志文件
    gbrain.log
```
...
| `gbrain config get <key>` | 获取单个配置值（可查询以下全部 23 项 key） |
| `gbrain config set <key> <value>` | 设置配置值（自动保存到 config.json） |

> **注意:** `config show` 仅显示最常用的 15 个核心 key；`config get <key>` 支持查询下表中列出的全部 23 个配置项。
...
| `post_write_lint` | boolean | 写入后运行 lint | `false` |
...
| `auto-low-risk` | `auto` | 自动接受低风险候选（实体提及、链接建议等），高风险仍需审核 |
...
| `promotion` | string | 否 | 提升策略: none / shadow / candidate / auto-low-risk(别名 auto) |
| `dry_run` | boolean | 否 | 仅返回路由计划，不实际写入 |
...
3. **嵌入** — 向量嵌入生成与持久化
4. **RAPTOR** — 递归摘要树（K-Means++ 聚类 + LLM 摘要，四级回退链：库级配置 → `GBRAIN_KB_RAPTOR_*` → `GBRAIN_EXPANSION_*` → `GBRAIN_CHUNKER_*`）
5. **持久化** — 事务保护的节点/向量写入

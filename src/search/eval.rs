...
impl Default for EvalConfig {
    fn default() -> Self {
        Self { strategy: EvalStrategy::Hybrid, rrf_k: 60, k: 5, expand: false }
    }
}

/// Search strategy for evaluation
#[derive(Debug, Clone, Copy)]
pub enum EvalStrategy { Hybrid, Keyword, Vector }
...
pub fn recall_at_k(relevant: &HashSet<String>, retrieved: &[String], k: usize) -> f64 {
    if relevant.is_empty() || k == 0 || retrieved.is_empty() { return 0.0; }
    let top_k = &retrieved[..k.min(retrieved.len())];
    let hits = top_k.iter().filter(|s| relevant.contains(*s)).count() as f64;
...
pub fn mrr(relevant: &HashSet<String>, retrieved: &[String]) -> f64 {
    for (i, doc) in retrieved.iter().enumerate() {
        if relevant.contains(doc) { return 1.0 / (i as f64 + 1.0); }
    }
    0.0
}
...
pub fn ndcg_at_k(relevant: &HashSet<String>, retrieved: &[String], k: usize) -> f64 {
    if relevant.is_empty() || k == 0 || retrieved.is_empty() { return 0.0; }
    let top_k = &retrieved[..k.min(retrieved.len())];
    let dcg: f64 = top_k.iter().enumerate().map(|(i, doc)| {
        let gain = if relevant.contains(doc) { 1.0 } else { 0.0 };
        if i == 0 { gain } else { gain / (((i + 2) as f64).ln() / std::f64::consts::LN_2) }
    }).sum();
    let ideal_dcg: f64 = (0..relevant.len().min(k)).map(|i| {
        let gain = 1.0;
        if i == 0 { gain } else { gain / (((i + 2) as f64).ln() / std::f64::consts::LN_2) }
    }).sum();
...
/// Run evaluation
pub fn run_eval(engine: &SqliteEngine, qrels: &[EvalQrel], config: &EvalConfig) -> Result<EvalReport> {
    let mut results = Vec::new();
    let mut sum_p = 0.0; let mut sum_r = 0.0; let mut sum_mrr = 0.0; let mut sum_ndcg = 0.0;
    let hybrid_opts = HybridOpts { rrf_k: config.rrf_k, ..HybridOpts::default() };

    for qrel in qrels {
        let relevant_set: HashSet<String> = qrel.relevant.iter().cloned().collect();
        let search_opts = SearchOpts { limit: Some(config.k * 3), detail_level: Some(DetailLevel::Medium), ..SearchOpts::default() };
        let hits = match config.strategy {
            ...
            EvalStrategy::Hybrid => hybrid_search(engine, &qrel.query, None, search_opts, hybrid_opts.clone())?,
        };
        ...
        sum_p += p; sum_r += r; sum_mrr += m; sum_ndcg += n;
        results.push(QueryResult { query: qrel.query.clone(), retrieved, p_at_k: p, r_at_k: r, mrr: m, ndcg_at_k: n });
    }
    let n = qrels.len().max(1) as f64;
    Ok(EvalReport { config_label: format!("{:?}", config.strategy), k: config.k, num_queries: qrels.len(), avg_p_at_k: sum_p / n, avg_r_at_k: sum_r / n, avg_mrr: sum_mrr / n, avg_ndcg_at_k: sum_ndcg / n, results })
}

/// A/B comparison
pub fn run_ab_eval(engine: &SqliteEngine, qrels: &[EvalQrel], config_a: &EvalConfig, config_b: &EvalConfig) -> Result<(EvalReport, EvalReport)> {
    Ok((run_eval(engine, qrels, config_a)?, run_eval(engine, qrels, config_b)?))
}

/// Parse qrels from JSON string or file path
pub fn parse_qrels(json: &str) -> std::result::Result<Vec<EvalQrel>, String> {
    if let Ok(qrels) = serde_json::from_str::<Vec<EvalQrel>>(json) { return Ok(qrels); }
...
mod tests {
    ...
    fn test_precision_at_k() {
        ...
        let retrieved: Vec<String> = ["a", "x", "b", "y", "c"].iter().map(|s| s.to_string()).collect();

...
| 代码元数据 | ✓ (language, symbol_name, symbol_type, start_line, end_line, parent_symbol_path, doc_comment, symbol_name_qualified) | 部分：language, symbol_name, symbol_type, start_line, end_line |
| embedding字段 | ✓ (Float32Array) | ✓（写入 `chunk_embeddings` fallback 表，`ChunkInput` 支持 embedding） |
| model字段 | ✓ | ✓ |
| embedded_at | ✓ | ✓ |
| StaleChunkRow | ✓ | ✓ (`StaleChunk`) |
...
| CodeEdgeInput | ✓ | ✓ | Rust 已有基础页内 calls/references |
| CodeEdgeResult | ✓ | ✓ (`CodeEdge`) | Rust 已有基础 callers/callees |
...
| two-pass retrieval | search/two-pass.ts | Rust 尚未实现；已有 code_edges 支撑 callers/callees |
...
| **Code parser** | ✓ (tree-sitter, 1,050行) | ✓（确定性解析 Rust/TS/JS/Python/Go/Java/C 风格声明） | 实现路线不同 |
| **Edge extractor** | ✓ (178行) | ✓（页内 calls/references） | Rust 更简单 |
| **Qualified names** | ✓ (109行) | ✓（container.method 风格） | Rust 更简单 |

Rust 已实现确定性 symbol chunk、qualified names、`chunks_fts` 和 `code_edges`；仍缺少 tree-sitter AST 级精度和 Cathedral II two-pass retrieval。
...
| 分块器/代码索引 | 78% | 已有确定性 code chunks 与 code_edges；仍缺少 tree-sitter 精度和 two-pass retrieval |
...
| P3 | 确定性 code chunker + code_edges | Done | 代码索引需求 |
